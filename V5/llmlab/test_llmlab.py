    ...
    def do_POST(self):
        ...
        if "code word" in prompt.lower():
            ...
            if "BASALT" in joined:
                ...
                text = "MERIDIAN"
        elif "It missed these rules" in prompt:
            text = 'The phrase "is important" is vague.\n\nWhich factor matters most?'
...
check("ladder has five stages", len(labels) == 5, labels)
check("stage order is prompt, skill, harness, loop, wiki",
      [l.split(" · ")[1] for l in labels] == ["prompt", "skill prompt", "harness prompt", "loop prompt", "wiki"], labels)
check("harness stage points at the command line",
      "python -m llmlab harness" in ladder["runs"][2]["text"])
check("stages one, two and four are checked against the rules",
      all("checks" in ladder["runs"][i] for i in (0, 1, 3)))
loop_notes = ladder["runs"][3]["notes"]
check("the loop leaves a log of rounds and a stop reason",
      loop_notes[0].startswith("round 1") and loop_notes[-1].startswith("stopped"), loop_notes)
check("the loop stops once the checks pass",
      "stopped: all rules met" in loop_notes and len(loop_notes) == 3, loop_notes)
check("the wiki stage reads its lessons back",
      "Lessons from earlier runs" in ladder["runs"][4].get("wiki", ""), ladder["runs"][4])
...
out_w = pathlib.Path("/tmp/llmlab_out_wiki")
if out_w.exists():
    shutil.rmtree(out_w)
wiki_file = out_w / "wiki.md"
log1 = []
w1 = run_harness(b, str(ROOT / "examples/feedback_spec.md"), str(ROOT / "examples/inputs"),
                 str(out_w), loop=3, wiki_path=str(wiki_file), log=log1)
head1 = pathlib.Path(w1[0]).read_text()
check("stage four: the header records the loop", "loop:" in head1 and "rounds used" in head1, head1[:400])
check("stage five: the wiki file is written with lessons",
      wiki_file.exists() and "\n- " in wiki_file.read_text(), log1)
log2 = []
w2 = run_harness(b, str(ROOT / "examples/feedback_spec.md"), str(ROOT / "examples/inputs"),
                 str(out_w), loop=3, wiki_path=str(wiki_file), log=log2)
head2 = pathlib.Path(w2[0]).read_text()
check("stage five: the next run reads the lessons back",
      "(0 lessons read)" not in head2 and "lessons read" in head2, head2[:400])
check("stage five: lessons are not written twice", log2[-1].endswith("0 added"), log2)
