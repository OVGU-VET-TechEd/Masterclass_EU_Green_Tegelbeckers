...
# which is the whole difference between rung three and rung four.
...
def lab_steer(
    ...
    **kw
    ...
    for label, r in (
        ...
        ("request plus standing instruction", standing),
    ):
    ...
    return {
        ...
        "claim": (
            "A standing instruction is worth more than a better-worded request — "
...
def lab_ladder(backend: Backend, rung: str = "all", **kw) -> Dict[str, Any]:
    sample = (
    ...
    if rung in ("all", "loop"):
        ...
        second = backend.generate(
            "Here is feedback on a student paragraph:\n%s\n\nCut it to two points. "
            "Remove every judgement of the writer. Keep the quotation."
            % first.text,
            temperature=0.0,
            seed=5,
            max_tokens=140,
        ...
        rungs.append(
            {
                "label": "2 · loop",
                "artefact": "a conversation you cannot repeat",
                "reusable": False,
                "checks": check_feedback(second.text),
                **second.as_dict(),
            }
        )

    if rung in ("all", "skill"):
        r = backend.generate(sample, system=LADDER_SPEC, temperature=0.0, seed=5, max_tokens=140)
        skill_checks = check_feedback(r.text)
        rungs.append(
            {
                "label": "3 · skill",
                "artefact": "a saved instruction, reusable and inspectable",
                "reusable": True,
                "spec": LADDER_SPEC,
                "checks": skill_checks,
                **r.as_dict(),
    ...
    if rung in ("all", "harness"):
        rungs.append(
            {
                "label": "4 · harness",
                "artefact": "a specification file run over a folder of inputs",
                "reusable": True,
                "text": (
                    "Not shown here, because the point of this rung is that it does "
                    "not live in a window. Run it from the command line:\n\n"
                    "    python -m llmlab harness --spec examples/feedback_spec.md "
                    "--inputs examples/inputs --out out\n\n"
                    "That command applies one written specification to every file "
                    "in a folder, writes one output file per input, and records "
                    "the model, the settings and the timestamp in each. It can be "
                    "versioned, reviewed, corrected and handed over."
                ),
                "prompt_tokens": None,
                "output_tokens": None,
                "seconds": 0.0,
    ...
    parts = []
    if skill_checks is not None:
        missed = [c["rule"] for c in skill_checks if not c["ok"]]
        parts.append(
            "Only rung three has its rules written down, so only rung three can "
            "be checked against them honestly. This run it met %d of %d%s"
            % (
                _met(skill_checks),
                len(skill_checks),
                "; it missed: %s. The rung did not make the output right. It made "
                "the miss visible, and fixable in one place." % "; ".join(missed)
                if missed
                else ".",
            )
        )
        if len(rungs) > 1:
            parts.append(
                "The other rungs were run against the same checks for comparison, "
                "though nobody gave them the rules — which is the position you are "
                "in whenever you review rung-one output against rules that exist "
                "only in your head."
            )
    parts.append(
        "What survives the session: nothing, nothing, one instruction, one repository."
    )
    return {
        "lab": "ladder",
        "title": "Four rungs, four artefacts",
        "claim": "The rung you work at determines what you still have next term.",
        "runs": rungs,
        "reading": " ".join(parts),
        "ask": "Which rung is your current practice on, and what does it leave behind?",
    }
...
BLOCK_MAP = {
    ...
    "steer": "Block 3.2 — the prompting ladder, rungs 1 to 3",
    "ground": "Block 1 — why outputs cannot be trusted",
    "ladder": "Block 3.2 and 3.4 — the ladder and the worked example",
