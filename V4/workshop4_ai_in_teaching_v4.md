...
--{{0}}--
Welcome. This is the self-study version of Workshop 4 in the EU GREEN Training
Lab series. It carries the same argument as the live session, in a straighter
...
<div style="color:#45566a;font-size:0.82em;">EU GREEN · Transformative Teaching and Learning · WP2 Training Lab · Workshop 4 · <strong>version 4</strong></div>
...
**If you are teaching from it.** The live version is `masterclass_ai_in_teaching_v4.html`
...
If Workshop 3 left you with Ollama and Gemma, this will find them. If nothing
answers, the tool assembles the text itself and says so on every screen — so a
broken laptop does not remove you from the exercise.
...
4. A dated 90-day plan with two areas on it, and no more than two —
   [the plan page](pages/p4_cpd_plan.html).
...
<iframe src="./pages/p1_self_check.html" width="100%" height="640"
...
- **GWDG academic services** — an EU-hosted assistant available to many German
  institutions through the existing research computing agreement. Its virtue is
  the agreement, not the model. Its fault: the model catalogue changes, so a
  prompt tuned last term may behave differently this term and you will not be
  told.
- **HAWKI** — an open-source front end an institution can self-host in front of
  whichever model it has licensed. Its virtue is that the interface, the logging
  and the retention rules are yours. Its fault: somebody has to run it, and that
  person is usually already fully occupied.
- **Local models via Ollama** — Workshop 3's stack. Nothing leaves the machine.
  Its faults are real: smaller models are meaningfully weaker at long documents
  and at languages other than English.
- **Commercial general assistants** — capable, quick, and outside your
  institution's control. The best are behind a subscription; see the equity
  section.
...
The gain is in **drafting** against a standing instruction you wrote and can
defend, never in the last step before it reaches the student.
...
<iframe src="./pages/p2_tool_triage.html" width="100%" height="640"
 style="border:1px solid #ddd;border-radius:8px"></iframe>
...
response that does work is a format in which the process is the evidence —
which is Workshop 5.
...
**Where to send people.** Students: DigiStudi (iMooX), or Elements of AI for a
colleague with no starting point at all. Staff: the twillo OER materials, which
are German, openly licensed and short enough to be read. Regionally: find out
whether your state has a structure like KI:Expertisezentrum.nrw before you build
anything.
...
| **Training vs inference** | weights are fixed when you use it; your session does not change them | You believe it "learns from" your corrections, so you correct repeatedly instead of writing the instruction down. You also fear the wrong risk. |
...
## 3.2 · The four-rung ladder

--{{0}}--
The rungs are not mainly a quality ladder: a single rung-one answer can be as
good as a rung-three one. They differ in what survives the session, and in
whether the output can be checked against anything written down.

| Rung | What it is | What it leaves behind |
| --- | --- | --- |
| **1 · Prompt** | one request, one answer | nothing |
| **2 · Loop** | correct, narrow, correct again | a chat you cannot repeat |
| **3 · Skill** | the standing instruction, written once and saved | something you can hand over |
| **4 · Harness** | a specification in a file, run over many inputs, versioned | a repository with a licence |

**Rung 1** is perfectly appropriate for a one-off. The error is not using rung
one; it is using rung one for something you will do forty more times.

**Rung 2** feels the most productive and transfers the least. Six exchanges
produce something usable and nothing reusable.

**Rung 3** is the move that matters. Name the audience, the scope, the count and
the prohibition. The example used in the lab: *address the writing, never the
writer; quote the words at issue; two points maximum; end with one answerable
question; do not rewrite the passage.* That instruction is a colleague-readable
artefact — reviewable, disputable, reusable next year by somebody else.

**Rung 4** adds one specification, a folder of inputs, one output per input, each
carrying a header recording the input, the specification, the model, the
temperature, the seed and the timestamp. Nothing about it is sophisticated. That
is the argument: the difference between rung one and rung four is not
cleverness, it is that this leaves a trail somebody else can audit.

<div style="border:1px solid #d8c8ee;border-left:5px solid #6b4a9e;border-radius:0 10px 10px 0;background:#f8f5fd;padding:12px 16px;margin:16px 0;">
<div style="font-weight:700;color:#6b4a9e;font-size:0.82em;letter-spacing:0.03em;">LAB 4 & 6 &middot; Climb the ladder, then run rung four</div>
<div style="color:#45566a;margin-top:5px;">Each run is checked against five of the written rules. Then open one of the files in out/: the provenance header at the top is the whole point; the text below it is secondary.</div>
</div>

```bash
# rungs 1 to 3 on one paragraph, each checked against the written rules
python -m llmlab lab ladder

# the same rules pasted into a request, or kept as a standing instruction
python -m llmlab lab steer

# rung 4 — one specification applied to a folder of inputs
...
That is the difference between rung three, where a person can check the rules,
and rung four, where anyone can run the checks.

## 3.3 · A worked example from the top rung
...
would have asked in week four rather than week eleven. That is a standing
instruction, not a prompt — which puts it on rung three.
...
Which of these is a rung-three artefact?
    ...
    *******************************************************************
    Rung three is the standing instruction: written once, readable by a
    colleague, reusable next year. The last option is rung four, which adds
    the folder, the provenance headers and the version history.
    *******************************************************************
...
| **Deepen** (5.2) | Started in two places: the tool decision record, and the standing instruction from rung three. | No peer coaching yet, and no use of data about your own practice to direct your development. |
...
If you would rather leave with something more specific than two areas, go to the
placement map below, run the practice you actually do most, and take the first
two items from the amber column.
...
<iframe src="./pages/p4_cpd_plan.html" width="100%" height="640"
 style="border:1px solid #ddd;border-radius:8px"></iframe>
...
- The rung you work at decides what still exists next term.
...
<p style="margin:12px 0 0;border-top:1px dashed #d6dee8;padding-top:9px;font-size:0.9em;"><strong>Legal floor:</strong> Removes the processor relationship and the transfer. Removes nothing else.<br><strong>In this course:</strong> Block 2 · Lab 7 (`route`) · Block 3 terminology table, deployment route.<br><strong>Read:</strong> Workshop 3 — AI Implementation Sprint · llmlab — seven exercises that check this session's claims on your own machine · [Regulation (EU) 2024/1689 (Artificial Intelligence Act)](https://eur-lex.europa.eu/eli/reg/2024/1689/oj)</p>
</div>
</details>
...
<p style="margin:12px 0 0;border-top:1px dashed #d6dee8;padding-top:9px;font-size:0.9em;"><strong>Legal floor:</strong> None directly, until the material is used to evaluate someone.<br><strong>In this course:</strong> Block 2 — preparation family · Block 3 — rung one and rung two.<br><strong>Read:</strong> [KI-Literacy — Grundlagen und Orientierung für Lehrende](https://www.twillo.de/edu-sharing/components/render/2e1ef4ad-15e0-4d17-af45-4c7223041966) · Self-learning nuggets: AI Orientation, AI Basics, AI Tools, Prompting, Quality and Ethics</p>
</div>
</details>

<details style="border:1px solid #d6dee8;border-left:4px solid #0f7a5a;border-radius:8px;margin:10px 0;padding:0;background:#ffffff;">
<summary style="cursor:pointer;padding:11px 14px;font-weight:600;color:#16202b;">I have written a standing instruction and I reuse it (rung three)</summary>
<div style="padding:0 14px 14px 14px;font-size:0.94em;color:#45566a;">
<p style="font-weight:600;color:#0f7a5a;margin:6px 0 4px;">A reasonable place to be</p>
...
<p style="margin:12px 0 0;border-top:1px dashed #d6dee8;padding-top:9px;font-size:0.9em;"><strong>Legal floor:</strong> None new. The instruction is what makes your human oversight demonstrable under Art. 14 if it ever matters.<br><strong>In this course:</strong> Block 3 — rung three · Lab 4 (`steer`).<br><strong>Read:</strong> llmlab — seven exercises that check this session's claims on your own machine · [KI als Reflexionsinstrument im wissenschaftlichen Arbeiten](https://zhq-blog.fh-aachen.de/kompetenzen-studierende/ki-als-reflexionsinstrument-im-wissenschaftlichen-arbeiten/)</p>
</div>
</details>

<details style="border:1px solid #d6dee8;border-left:4px solid #0f7a5a;border-radius:8px;margin:10px 0;padding:0;background:#ffffff;">
<summary style="cursor:pointer;padding:11px 14px;font-weight:600;color:#16202b;">I run a specification over a folder of inputs with provenance headers (rung four)</summary>
<div style="padding:0 14px 14px 14px;font-size:0.94em;color:#45566a;">
<p style="font-weight:600;color:#0f7a5a;margin:6px 0 4px;">A reasonable place to be</p>
<p style="margin:0 0 12px;color:#16202b;">Advanced on the prompting competence, and the only rung that leaves an auditable trail.</p>
<table style="width:100%;border:0;"><tr style="vertical-align:top;">
...
| `HET-C1` **Use AI to explore perspectives that build upon original academic ideas** | Foundation | Block 2 preparation family; Block 3 rungs one and two | — |
...
| `HET-C3` **Direct and iterate with generative AI systems (prompting as a documented practice)** | Applied | Block 3 — the four-rung ladder; Labs 4 and 6; the harness command | This is the spine of Block 3 and the clearest route to Applied. |
...
| `HET-M3` **Decompose a task to determine where AI should automate and where it should only augment** | Applied | Block 3 rung three; Block 4 — what will you keep doing by hand | Directly reinforced by Ferdman's capacity-hostile environments. |
...
| `HET-S3` **Design AI use attentive to data sources, data selection and information flow** | Applied | Block 2 deployment routes; Block 3 route row; Workshop 2 in full | — |
...
| `HET-X2` **Practise scholarly work with AI** | Foundation | Block 3.4 provenance and licensing; Block 1 fabrication | Scholarly writing with AI is a Workshop 5 and doctoral-supervision topic. |
...
1. EduGreenLabs / OvGU Magdeburg (2026). *Workshop 2 — Privacy by Design.* WP2 Training Lab, EU GREEN Alliance. CC BY 4.0.
1. EduGreenLabs / OvGU Magdeburg (2026). *Workshop 3 — AI Implementation Sprint.* Installs the local stack (Ollama, Gemma) this session's lab uses. CC BY 4.0.
...
1. EduGreenLabs / OvGU Magdeburg (in preparation). *Workshop 5 — Assessment in the age of generative AI.*
...
**Companion files:** `masterclass_ai_in_teaching_v4.html` (the live session
